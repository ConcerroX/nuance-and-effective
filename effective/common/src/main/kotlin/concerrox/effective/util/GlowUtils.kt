...
object GlowUtils {
    ...
    fun isGlowingWater(level: Level, pos: BlockPos): Boolean {
        return EffectiveConfig.glowingPlankton.get() && level.isNightTime && level.getBiome(pos)
            .`is`(Biomes.WARM_OCEAN)
    }

    fun getGlowingWaterColor(level: Level, pos: BlockPos): Color {
        return Color(min(1F, level.random.nextFloat() / 5F + level.getBrightness(LightLayer.BLOCK, pos) / 15F),
